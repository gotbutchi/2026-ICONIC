...
| --- | --- | --- |
| **1. Data Quality Audit** | Identified and resolved 3 critical source issues in `stg_weekly_sales.sql`:<br><br>1. **The Phantom Week Bug**: Resolved invalid date `14/13/2019`. A naive `REPLACE` to Dec 14th would yield a Saturday, corrupting downstream `LAG()` logic. Recovered the true date (`14/06/2019`) with 100% certainty via macroeconomic interpolation (matching CPI & Unemployment) against Store 42's Friday sequence.<br><br>2. Flagged zero/negative sales outliers (`is_invalid_sales`).<br><br>3. Enforced safe type casting & Null handling (`COALESCE`). | **DONE** |
